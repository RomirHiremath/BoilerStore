...
const categoryColors = {
  ...
  "Dorm Essentials": "bg-amber-100 text-amber-800",
...
export default function ListingDetailModal({ listing, onClose, startContact, startTrade }) {
  ...
  const handleShare = async () => {
    const shareData = {
      ...
      text: `Check out this item on Boiler Exchange: ${currentListing.title} - $${currentListing.price}`,
                    ...
                    {currentListing.images.map((img, index) => (
                      <button 
                        ...
                        className={`w-20 h-20 rounded-md overflow-hidden border-2 flex-shrink-0 ${
                          selectedImage === index ? 'border-[--purdue-gold]' : 'border-transparent'
                        }`}
                      >
            ...
            <div className="bg-gray-50/70 border-l p-4 md:p-6 space-y-6">
              <Card className="bg-white">
                <CardContent className="p-6">
                  ...
                  <p className="text-4xl font-extrabold text-black mt-1 mb-6">${currentListing.price}</p>
                  <div className="space-y-3">
                    <motion.div whileTap={{ scale: 0.98 }}>
                      <Button 
                        size="lg" 
                        className="w-full bg-black hover:bg-gray-800 text-white font-bold text-lg py-6" 
                    ...
                    <motion.div whileTap={{ scale: 0.98 }}>
                      <Button 
                        size="lg" 
                        variant="outline"
                        className="w-full border-2 border-black text-black hover:bg-gray-100 font-bold text-lg py-6" 
                ...
                <CardContent>
                  {isLoadingSeller ? (
                    ...
                    <div className="flex items-center gap-4 group">
                      ...
                      <div>
                        <p className="font-bold text-lg group-hover:text-black">
                          {seller.full_name}
                        </p>
