...
export default function Layout({ children }) {
  ...
  useEffect(() => {
    document.title = "Boiler Exchange";
  }, []);
  ...
  return (
      <div className="min-h-screen bg-white font-sans">
        ...
        <header className="bg-[--purdue-black] shadow-md sticky top-0 z-40">
          <div className="container mx-auto px-4">
            <div className="flex items-center justify-between h-16">
              <Link to={createPageUrl("Dashboard")} className="flex items-center gap-3 hover:opacity-80 transition-opacity">
                <img 
                  src="https://qtrypzzcjebvfcihiynt.supabase.co/storage/v1/object/public/base44-prod/public/a7ec8b476_Purdue-University-logo.png" 
              ...
              <div className="flex items-center gap-4">
                {currentUser ? (
                    ...
                    <DropdownMenuTrigger asChild>
                      <button className="flex items-center gap-2 text-sm font-medium focus:outline-none hover:opacity-80 transition-opacity">
                        <Avatar className="w-9 h-9 ring-2 ring-white/50">
                          ...
                          <AvatarFallback className="bg-[--purdue-gold] text-[--purdue-black] font-bold">
                            {getInitials(currentUser.full_name)}
                          </AvatarFallback>
                        </Avatar>
                        <span className="text-white hidden sm:block font-semibold">Boiler Up!</span>
                      </button>
                    </DropdownMenuTrigger>
                    <DropdownMenuContent align="end" className="w-56 bg-white border-gray-200">
                      <DropdownMenuLabel>
                        <p className="font-bold text-gray-900">{currentUser.full_name}</p>
                        <p className="text-xs text-gray-600 font-normal">{currentUser.email}</p>
                      </DropdownMenuLabel>
                      <DropdownMenuSeparator />
                  ...
                  <Button onClick={User.login} className="bg-[--purdue-gold] hover:bg-amber-500 text-[--purdue-black] font-bold border-2 border-[--purdue-black]">
        ...
        {mobileMenuOpen && (
          <div className="fixed inset-0 z-50 bg-black/60 md:hidden" onClick={() => setMobileMenuOpen(false)}>
              <div className="fixed top-0 right-0 h-full w-4/5 max-w-sm p-6 shadow-xl bg-[--purdue-black]" onClick={(e) => e.stopPropagation()}>
                  <div className="flex justify-between items-center mb-8">
                    <img 
                      ...
                      className="h-20"
                    />
                    <button onClick={() => setMobileMenuOpen(false)} className="text-white hover:text-gray-300">
                      <X className="w-7 h-7" />
                    </button>
                  </div>
                  ...
                  <div className="absolute bottom-8 left-0 right-0 px-6">
                    <div className="text-center text-gray-400">
                      <p className="font-medium">Boiler Up! 🚂</p>
                      <p className="text-sm mt-1">Purdue Pride</p>
                    </div>
                  </div>
              </div>
          </div>
        ...
        <footer className="bg-[--purdue-black] mt-16">
          <div className="container mx-auto py-6 px-4 flex items-center justify-between text-white">
            ...
            <div>
              <p className="font-bold text-lg text-right">Boiler Up! 🚂</p>
              <p className="text-gray-400 text-sm text-right">&copy; {new Date().getFullYear()} Boiler Exchange</p>
            </div>
          </div>
        </footer>
      </div>
